"""
Step 1 as a long-lived worker: claim a document, parse it, publish a sha.

Same work as `steps.parse`, cut differently. The batch step is handed a
directory and walks it; this one is handed a single document at a time and has
no idea how many exist. That is the whole difference, and it is what lets N of
these run at once without any of them agreeing on anything.
...
def handle(
    ...
    mime = request.mime or mimetypes.guess_type(name)[0] or DEFAULT_MIME
    manifest = Manifest.now(sha, uri, mime, settings.parser_version)
...
def run(
    ...
    *,
    ...
    """Consume `to-parse` until it goes quiet.

    Queues can be injected, which is how the tests run a whole round trip in
    one process against `memory://` without touching argv or the environment.
    """
    ...
    log.info(
        "parse worker done: %d parsed, %d failed, %d dead-lettered",
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        ...
        description="Step 1 as a queue worker: one document per message.",
    )
    ...
    # A dead letter on its own is not a worker failure — the message is parked,
    # the worker is healthy, and restarting the pod would only re-park it.
    # Handling nothing successfully is different: that is a bad mount or a bad
    # config, and it should be loud.
